...
@section('content')
    <div class="mb-6">
        <a href="{{ route('admin.messages.index') }}" class="text-secondary hover:text-text-primary text-sm font-semibold flex items-center gap-2"><i class="fas fa-arrow-left"></i> Back to Inbox</a>
    </div>
    ...
    <div class="admin-card max-w-3xl">
        <div class="flex flex-col gap-6">
            <div class="flex justify-between items-start border-b border-glass-border/30 pb-4">
                <div>
                    <h3 class="text-xl font-bold text-text-primary mb-1">{{ $message->name }}</h3>
            ...
            <div>
                ...
                <p class="text-text-primary text-md font-semibold">{{ $message->subject ?? '(No Subject Specified)' }}</p>
            </div>
            ...
            <div class="border-t border-glass-border/30 pt-6 flex justify-between items-center">
                ...
                <form method="POST" action="{{ route('admin.messages.destroy', $message->id) }}" onsubmit="return confirm('Are you sure you want to delete this message?');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn-secondary py-2.5 px-6 border-accent-coral/30 text-accent-coral hover:bg-accent-coral hover:text-text-primary flex items-center gap-2">
