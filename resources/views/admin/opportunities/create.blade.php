...
@section('page_title', 'Add Opportunity')

@section('content')
    <div class="mb-6">
        <a href="{{ route('admin.opportunities.index') }}" class="text-secondary hover:text-text-primary text-sm font-semibold flex items-center gap-2"><i class="fas fa-arrow-left"></i> Back to Opportunities</a>
    </div>
        ...
        <form method="POST" action="{{ route('admin.opportunities.store') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" id="title" name="title" class="form-input" placeholder="e.g. Master's Thesis Grant 2026" value="{{ old('title') }}" required>
                </div>
                <div>
                    <label for="type" class="form-label">Type</label>
                    <select id="type" name="type" class="form-input">
                        <option value="thesis_grant">Thesis / Research Grant</option>
            ...
            <button type="submit" class="btn-primary py-3 px-6">
                <i class="fas fa-save mr-2"></i> Save Opportunity
            </button>
        </form>
    </div>
@endsection
