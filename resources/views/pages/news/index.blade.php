...
@section('content')
    ...
    <header class="page-header">
        <div class="container mx-auto px-4 md:px-8 relative z-10">
            ...
            <h1 class="text-4xl md:text-5xl font-heading font-extrabold text-white">News & Events</h1>
        ...
        <div class="container mx-auto px-4 md:px-8">
            ...
            <div class="flex flex-col lg:flex-row justify-between items-center mb-16 animate-on-scroll gap-6 max-w-5xl mx-auto">
                ...
                <div class="glass p-1.5 rounded-xl flex gap-2 overflow-x-auto w-full lg:w-auto pb-1">
                    <a href="{{ route('news.index', ['search' => request('search')]) }}" class="px-6 py-2.5 rounded-lg text-sm font-semibold transition duration-300 whitespace-nowrap {{ empty(request('type')) ? 'bg-primary text-white shadow' : 'text-text-secondary hover:text-white' }}">
                        All Updates
                    </a>
                    <a href="{{ route('news.index', ['type' => 'news', 'search' => request('search')]) }}" class="px-6 py-2.5 rounded-lg text-sm font-semibold transition duration-300 whitespace-nowrap {{ request('type') === 'news' ? 'bg-primary text-white shadow' : 'text-text-secondary hover:text-white' }}">
                        News
                    </a>
                    <a href="{{ route('news.index', ['type' => 'event', 'search' => request('search')]) }}" class="px-6 py-2.5 rounded-lg text-sm font-semibold transition duration-300 whitespace-nowrap {{ request('type') === 'event' ? 'bg-primary text-white shadow' : 'text-text-secondary hover:text-white' }}">
                        Events
                    </a>
                </div>
                ...
                <form action="{{ route('news.index') }}" method="GET" class="relative w-full lg:w-auto">
                    ...
                    @endif
                    <input type="text" name="search" placeholder="Search news & events..." value="{{ request('search') }}" class="w-full lg:w-80 bg-glass-light/10 border border-glass-border/30 rounded-xl py-3 px-5 text-white focus:outline-none focus:border-secondary focus:ring-1 focus:ring-secondary transition placeholder-text-muted">
                ...
                @forelse($articles as $article)
                    <div class="glass-card flex flex-col h-full overflow-hidden animate-on-scroll">
                        <div class="h-48 bg-primary-light/5 overflow-hidden relative">
                            ...
                            @endif
                            <div class="absolute top-4 left-4 bg-primary-accent text-white text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                                {{ $article->type }}
                            </div>
                        </div>
                        <div class="p-6 flex flex-col flex-grow">
                            ...
                            @endif
                            <a href="/news-and-events/{{ $article->slug }}" class="text-secondary hover:text-white text-xs font-semibold flex items-center gap-1.5 mt-auto transition">
